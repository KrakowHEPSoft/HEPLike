...
void HL_nDimGaussian::Read()
{
  if(! initialized)
  {
    throw std::runtime_error("HL_nDimGaussian, TRYING TO READ WITHOUT GIVING ANY FILE!");
    return;
  }
  read_standard();
    ...
    for(YAML::const_iterator it = node.begin(); it != node.end();  ++it )
    {
      if(row==0)
      {
        for(int i=0;i<NoOfObservables; i++)
        {
          ...
          {
            throw std::runtime_error("Error in HL_nDimGaussian, wrong ordering of names in correlation matrix");
          }
        }
      }// row=0
      else
...
double HL_nDimGaussian::GetChi2(std::vector<double> theory , boost::numeric::ublas::matrix<double> theory_cov)
{
  ...
  {
    throw std::runtime_error("Error in HL_nDimGaussian::GetChi2, you had different dimensions in theory and cov matrix");
  }
  if(theory_cov.size2() != theory_cov.size1() )
  {
    throw std::runtime_error("Error in HL_nDimGaussian::GetChi2, your theory cov matrix is not square!");
  }
