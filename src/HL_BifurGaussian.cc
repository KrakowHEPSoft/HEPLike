...
void HL_BifurGaussian::Read()
{
  if(! initialized)
  {
    throw std::runtime_error("HL_BifurGaussian Warning, TRYING TO READ WITHOUT GIVING ANY FILE!");
  }
  ...
  if(config["Observables"])
  {
    ...
    if(HL_Sigma_stat_L<0. || HL_Sigma_syst_L <0.) throw std::runtime_error("HL_BifurGaussian warning, at least one of your error has wrong sign");

  }
  else
  {
    throw std::runtime_error("Error in the HL_BifurGaussian class, your yaml file has missing Observables");
  }
